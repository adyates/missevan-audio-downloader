...
class MissevanAPI:
    ...
    def get_audio_url(self, episode_id):
        ...
        if "redirect" in episode_info["info"].keys():
            sound_url = self.get_audio_url(episode_info["info"]["redirect"])
            return sound_url
    ...
    def get_image_url(self, episode_id):
        url = f"{BASE_URL}/sound/getimages?soundid={episode_id}"
        image_info = self._call_web_service(url)

        return image_info.get("successVal", {}).get("images", [[None]])[0][0]
